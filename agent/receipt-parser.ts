...
8. classification: ALWAYS classify the image. outcome "valid": you can clearly read a vendor AND an amount — printed or handwritten, any document type, even without a number. outcome "multiple": the image contains MORE THAN ONE distinct document/receipt — even if the second one is partially visible or cropped at the edge; never attempt extraction of either, return "multiple". outcome "borderline": the image shows a single document but it is blurry, cropped, or low-contrast, so you cannot read key fields. outcome "invalid": blank page, illegible scribble, or nothing traceable — no vendor, no amount, no number (e.g. a photo of a person or object). When in doubt between "multiple" and "borderline": if you can see two or more separate documents, choose "multiple"; otherwise choose "borderline" — never "invalid". A hand or fingers holding the receipt does NOT count as a second document.
