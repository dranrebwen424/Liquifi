...
assert.match(profile, /General information/, "profile must use one general-information section");
assert.match(profile, /max-w-3xl/, "profile must stay single-column on desktop");
assert.doesNotMatch(profile, /lg:grid-cols/, "profile must not split into desktop columns");
assert.match(profile, /bg-surface-inverse/, "identity card must keep the reference-style header band");
